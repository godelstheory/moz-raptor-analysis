...
df_ps %>% filter(run_suite %in% first_build_run_suites) %>%
...
ggplot(br_ps_fcp_lt_f, aes(date, score)) +
  ...
  theme_bw() +
  labs(x = 'Build Date', y = 'Score: Load Time * FCP', title = 'End of May Reference: Windows 10-64 Build Scores') 
```
...
* Every new build fires off a set of `run_suite` page load tests for each platform (e.g., Windows 10-64).
     + A `run_suite` is a serialized snapshot of a web page (e.g., Amazon, Facebook) that is played back via [Mitmproxy](https://wiki.mozilla.org/Performance_sheriffing/Raptor/Mitmproxy).
...
For the following analysis, we choose a set of five builds from the end of the observation window (end of May). As noted above, a set of the most common `r length(unique(df_ps_agg1$run_suite))` `run_suite` for the builds throughout the observation window were used.
...
ggplot(df_ps_agg1, aes(run_datetime, mean_p_fcp_lt)) +
  ...
  theme_bw() + 
  labs(y = 'Score: Load Time * FCP', x = 'Build Date', title = 'End of May Reference: Windows 10-64 p score')
```
...
ggplot(df_ps_agg1_long, aes(run_datetime, measure)) + 
  ...
  theme_bw() + 
  theme(legend.position="top") + 
  labs(x = 'Build Date', y = 'Score', title = 'End of May Reference: Windows 10-64 Build Scores')
```
...
long_files <- c('data/activedata_125d_v5_windows10_aarch64_long.csv', 'data/activedata_125d_v5_windows10_64_shippable_long.csv',
  ...
  print(ggplot(br_ps_fcp_lt_f, aes(date, score)) +
    ...
    geom_ribbon(aes(ymin=score-sd, ymax=score+sd), alpha=0.4) + 
    theme_bw() +
    labs(x = 'Build Date', y = 'Score: Load Time * FCP', title = paste('End of May Reference:', platform)) )
}
```
